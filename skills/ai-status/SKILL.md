...
4. **Model routing.** Report the ladder of the runtime you are actually running
   ...
   | EXPERT | the `model:` pinned in `ai-expert.md` | the `model` pinned in `ai-expert.toml` |

   If the session model is above its tier's default (Claude `model` not sonnet,
   Codex `model` not `gpt-5.6-sol`), say so. When EXPERT is the escalation model
   and the runtime's gate reports `active`, EXPERT is running one tier down right
   now — say until when, and why. A Codex install has no `fable-gate`, a Claude
   install has no `codex-model-gate`; report only the one that exists.
