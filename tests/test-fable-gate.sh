...
grep -qx 'model: fable' "$D/agents/ai-expert.md" && pass "ai-expert keeps model: fable (the gate reroutes at run time)" || fail "ai-expert should stay pinned to fable"
CLAUDE_DIR="$D" bash "$INSTALL" --plan max --fable yes >/dev/null 2>&1
...
for opt in "max no" "pro no"; do
    ...
    grep -qx 'model: opus' "$D/agents/ai-expert.md" && pass "--plan $1 --fable $2 pins ai-expert to opus" || fail "ai-expert should be opus"
done
