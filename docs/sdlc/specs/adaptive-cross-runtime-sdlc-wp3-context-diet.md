...
| R13 | A managed block hand-edited inside the markers is never overwritten, except by an approved `--adopt` split (WP6 R7/R17), which first moves every hand-written line out and keeps the original in `.ai/reports/adopt-<date>/original/`: today's `block_update` conflict path applies (project file kept, plugin stub in `.ai/local/plugin-update/<file>`), listed on every run, plus a permanent hint naming the byte count and the fix. | C-no-overwrite |
