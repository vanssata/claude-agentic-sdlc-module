...
| 5 | Runtimes and plans | new `profiles/max20.json`, preferred-runtime and budget tables in every profile, `install.sh` plan detection + confirmation, `fable-gate` / `codex-model-gate` → `runtime-gate`, `state.py handoff --to`, grep test that shared prompts name no model | T3 | 2 | **done** (`spec/wp5-runtimes-and-plans`, 2026-09-21) — `claude_agentic` tables in every profile and `scripts/resolve-profile.py`, `max20`, plan detection with propose/confirm and `profile.json`, the agent sources as tier templates (`ai-reviewer-balanced` on Codex), `runtime-gate` (reroute, quota, EXPERT and fan-out budgets, `model_fallback`) on every plan with the old gates as shims, `state.py handoff --to`/`profile`/exit 7, migration 0005 (schema 5), `usage-report --task/--budgets`, and the model-free shared prompts; spec and plan under `…-wp5-runtimes-and-plans.md`, all seven open questions settled with the user, R18 over R16 by the user's decision |
