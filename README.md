...
| `agents/ai-expert.md.tmpl` | `ai-expert.md`, model per plan | `ai-expert.toml`, pinned to Astra |
...
| EXPERT | Fable 5.1 / Opus 5, `xhigh` | Astra, `xhigh` | T5, irreversible design, what STRONG could not settle |
...
triggers are listed in the managed instruction block.
...
| `fable-gate.py` | Claude, Fable installs only | `StopFailure` / `PreToolUse:Agent` | records a Fable rate limit or model-not-found, and rewrites `model: fable` to `opus` until the reset |
...
Fifteen suites, 599 assertions. The guards against JSON fixtures in both runtimes
(including `apply_patch` payloads that touch several files at once); the state
machine; scaffold idempotency for one runtime, the other, and both over a single
`.ai/` tree; installer rendering for every plan combination and every `--target`;
the Codex agent renderer, asserting each role's *effective* model and effort
rather than what was asked for; the `config.toml` merge against a fixture full of
third-party settings, and its refusal on a malformed file; the migration off
`claude-routing`; an end-to-end run that installs both runtimes into scratch
directories, scaffolds a throwaway repository, drives a T4 task through the
guards from Claude and then from Codex, and proves both see the same state; the
usage report's per-response deduplication for Claude and its per-record
accounting for Codex; and both escalation gates through every event.
